...
这条路径默认跳过 Playwright，依赖更少，最适合第一次确认项目能跑通。
...
- 创建 `pornclaw/.venv`
- 安装 `pornclaw/requirements.txt`
- 初始化数据库
- 尝试安装 Playwright Chromium
- 启动 `uvicorn app.main:app --reload`

如果你只是想先跑通 demo，也可以显式跳过 Playwright：
...
<details>
<summary><strong>手动模式（默认不需要，点开才看）</strong></summary>

如果你确实想手动控制每一步，仍然可以使用原来的命令：
